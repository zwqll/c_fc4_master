...
MODEL_PATH = 'data/squeeze_net/new.pkl'
...
class SqueezeNet(object):
  ...
  def build_model(self):
    net = {}
    self.net = net
    self.model = pkl.load(open(MODEL_PATH, 'rb'),encoding='bytes')
    ...
    net['conv1'] = self.conv_layer(
        ...
        W=self.weight_variable(
            [3, 3, 3, 64],
            name='conv1_w',
            init=np.transpose(self.model[b'conv1_weights'], [2, 3, 1, 0])),
        stride=[1, 2, 2, 1],
        padding='VALID') + self.model[b'conv1_bias'][None, None, None, :]
    ...
    net['conv10'] = self.conv_layer(
        ...
        W=self.weight_variable(
            [1, 1, 512, 1000],
            name='conv10',
            init=np.transpose(self.model[b'conv10_weights'], [2, 3, 1, 0])),
        padding='VALID') + self.model[b'conv10_bias'][None, None, None, :]
  ...
  def fire_module(self,
    ...
    s1_weight = self.weight_variable(
        ...
        np.transpose(
            self.model[(layer_name + '/' + 'squeeze1x1_weights').encode(encoding='utf-8')],
            axes=[2, 3, 1, 0]))
    ...
    e1_weight = self.weight_variable(
        ...
        np.transpose(
            self.model[(layer_name + '/' + 'expand1x1_weights').encode(encoding='utf-8')],
            axes=[2, 3, 1, 0]))
    e3_weight = self.weight_variable(
        [3, 3, s1x1, e3x3], layer_name + '_e3',
        np.transpose(
            self.model[(layer_name + '/' + 'expand3x3_weights').encode(encoding='utf-8')],
            axes=[2, 3, 1, 0]))
    ...
    fire['concat'] = tf.concat([
        tf.add(fire['e1'],
               self.bias_variable(
                   ...
                   value=self.model[(layer_name + '/' + 'expand1x1_bias').encode(encoding='utf-8')])),
        tf.add(fire['e3'],
               self.bias_variable(
                   [e3x3],
